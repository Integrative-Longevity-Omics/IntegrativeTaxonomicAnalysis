...
temp_renorm <- function(taxadata = TAX, otudata=OTU, rank=rank){
  #pseudo count: calculate minimum value of otu table and divide by two
  min.pseudo <- function(x){min(x[x > 0], na.rm=TRUE)/2}
  pseudo <- min.pseudo(otudata)
  # Apply pseudocount only to the zeros
  otudata[otudata == 0] <- pseudo
  #renormalize otu abundance
  renorm.OTU <- t(t(otudata)/colSums(otudata))
  
  prop.rel <- count_mat(taxadata = taxadata, otudata=renorm.OTU, rank=rank)
...
# Sample.ID Age_at_sample my.famID pn5_yrs_education
