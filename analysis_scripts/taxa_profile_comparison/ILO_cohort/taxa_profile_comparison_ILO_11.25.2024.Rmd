...
output_path <- "/restricted/projectnb/uh2-sebas/analysis/metagenomics/parallel_systems_comparison/01_taxa_profile_comparison/ILO_cohort/output/"
meta.dir <-"/restricted/projectnb/uh2-sebas/data/metagenomics/ILO_combined_cohort/decoder/"
...
bracken_R0_biom <- lapply(1:length(kmer_thresh), function(x){
  #load phyloseq obj
  bracken_ilo_biom <- readRDS(paste0(flpath, "bracken_renorm_ILO_phyloseq.R0.",kmer_thresh[x],".10.09.2024.rds"))
...
bracken_R0.5_biom <- lapply(1:length(kmer_thresh), function(x){
  #load phyloseq obj
  bracken_ilo_biom <- readRDS(paste0(flpath, "bracken_renorm_ILO_phyloseq.R0.5.",kmer_thresh[x],".10.09.2024.rds"))
...
bracken_R1_biom <- lapply(1:length(kmer_thresh), function(x){
  #load phyloseq obj
  bracken_ilo_biom <- readRDS(paste0(flpath, "bracken_renorm_ILO_phyloseq.R1.",kmer_thresh[x],".10.09.2024.rds"))
...
lapply(1:length(kmer_thresh), function(x){
  ...
  common.relabund.metaphlan <- colSums(metaphlan_ilo_biom.otu[[1]])
  unique.relabund.metaphlan <- 1-common.relabund.metaphlan
  common.relabund.bracken <- colSums(bracken_ilo_biom.otu[[1]])
...
lapply(1:length(kmer_thresh), function(x){
  #calculate total relative abundances for in-common taxa and unique taxa based on subsetted otu table with in-common taxa from above
  common.relabund.metaphlan <- colSums(metaphlan_ilo_biom.otu[[2]])
  unique.relabund.metaphlan <- 1-common.relabund.metaphlan
  common.relabund.bracken <- colSums(bracken_ilo_biom.otu[[2]])
  unique.relabund.bracken <- 1-common.relabund.bracken
  #boxplot(common.relabund.metaphlan, xlab="metaphlan", ylab="relative abundances of common taxa", main="relative abundance per sample")
  #boxplot(common.relabund.bracken, xlab="bracken", ylab="relative abundances of common taxa", main="relative abundance per sample")
...
#save figure
png(paste0(paste0(output_path,"Figures/stacked_barplot_species_ilo.png")), units="in", width=10, height=5, res=300)
p.rel.common
dev.off()
```
...
```{r, fig.dim=c(20,5)}
#save figure
png(paste0(paste0(output_path,"Figures/top10_abundant_species_ilo.png")), units="in", width=20, height=5, res=300)
