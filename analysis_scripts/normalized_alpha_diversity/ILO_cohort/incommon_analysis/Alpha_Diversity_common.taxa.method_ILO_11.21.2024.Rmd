---
title: "Alpha diversity analysis with ILO cohort data based in-common taxa between taxonomic classification methods"
...
common.relabund.metaphlan <- colSums(metaphlan_ilo_biom.otu)
common.relabund.bracken <- colSums(bracken_ilo_biom.otu)
...
# colnames(metaphlan_ilo_renorm.otu) <- paste0(colnames(metaphlan_ilo_renorm.otu), "_Metaphlan")
# colnames(bracken_ilo_renorm.otu) <- paste0(colnames(bracken_ilo_renorm.otu), "_Bracken")
...
OTU_species <- metaphlan_ilo_renorm.otu
...
OTU_genus <- metaphlan_ilo_renorm.otu
...
OTU_family <- metaphlan_ilo_renorm.otu
...
OTU_order <- metaphlan_ilo_renorm.otu
...
OTU_class <- metaphlan_ilo_renorm.otu
...
OTU_phylum <- metaphlan_ilo_renorm.otu
...
write_csv(metaphlan.ctds.list, file=paste0(outpath,"entropy.metaphlan.commontaxa.ilo.csv"))
...
```{r}
#species level
lm.species <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.species)
...
#genus level
lm.genus <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.genus)
...
#family level
lm.family <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.family)
...
#order level
lm.order <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.order)
...
#class level
lm.class <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.class)
...
#phylum level
lm.phylum <- lm(statistic ~ Age_at_FecalSample, data=metaphlan.ctds.phylum)
...
```{r, fig.dim = c(50,10)}
#For species level
p.s_age <- ggplot(metaphlan.ctds.species, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
...
#For genus level
p.g_age <- ggplot(metaphlan.ctds.genus, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
    ylim(-1,0) +
  xlim(50,110) +
...
#For family level
p.f_age <- ggplot(metaphlan.ctds.family, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
...
#For order level
p.o_age <- ggplot(metaphlan.ctds.order, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() +
  geom_smooth(method="lm") +
    ylim(-1,0) + 
  xlim(50,110) +
...
#For class level
p.c_age <- ggplot(metaphlan.ctds.class, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
    ylim(-1,0) + 
  xlim(50,110) +
...
#For phylum level
p.p_age <- ggplot(metaphlan.ctds.phylum, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() +
  geom_smooth(method="lm") +
  ylab("normalized alpha diversity") +
    ylim(-1,0) + 
...
png(paste0(outpath,"Figures/incommon_taxa/entropy_metaphlan_incommon.taxa_ilo_alllevels.png"), units="in", width=30, height=20, res=300)
ggpubr::ggarrange(p.p_age, p.c_age, p.o_age, p.f_age, p.g_age, p.s_age,
           ncol = 3, nrow = 2)
 dev.off()
 
png(paste0(outpath,"Figures/incommon_taxa/entropy_metaphlan_incommon.taxa_ilo_main.png"), units="in", width=30, height=10, res=300)
...
```{r, warning=FALSE}
#taxa diversity for species
OTU_species <- bracken_ilo_renorm.otu
...
#taxa diversity for genus
OTU_genus <- bracken_ilo_renorm.otu
...
#taxa diversity for family
OTU_family <- bracken_ilo_renorm.otu
...
#taxa diversity for order
OTU_order <- bracken_ilo_renorm.otu
...
#taxa diversity for class
OTU_class <- bracken_ilo_renorm.otu
...
#taxa diversity for phylum
OTU_phylum <- bracken_ilo_renorm.otu
...
write_csv(bracken.ctds.list, file=paste0(outpath,"entropy.bracken.commontaxa.ilo.csv"))
...
```{r}
#species level
lm.species <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.species)
pval.species <- broom::tidy(lm.species) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.species <- broom::tidy(lm.species) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
...
#genus level
lm.genus <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.genus)
pval.genus <- broom::tidy(lm.genus) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.genus <- broom::tidy(lm.genus) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
...
#family level
lm.family <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.family)
pval.family <- broom::tidy(lm.family) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.family <- broom::tidy(lm.family) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
...
#order level
lm.order <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.order)
pval.order <- broom::tidy(lm.order) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.order <- broom::tidy(lm.order) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
...
#class level
lm.class <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.class)
pval.class <- broom::tidy(lm.class) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.class <- broom::tidy(lm.class) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
summary(lm.class)

#phylum level
lm.phylum <- lm(statistic ~ Age_at_FecalSample, data=bracken.ctds.phylum)
pval.phylum <- broom::tidy(lm.phylum) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(p.value) %>% signif(3)
est.phylum <- broom::tidy(lm.phylum) %>% dplyr::filter(term=="Age_at_FecalSample") %>% pull(estimate) %>% signif(3)
...
```{r, fig.dim = c(50,10)}
#For species level
p.s_age <- ggplot(bracken.ctds.species, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
    ylim(-1,0) + 
  xlim(50,110) +
...
#For genus level
p.g_age <- ggplot(bracken.ctds.genus, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
    ylim(-1,0) +
  xlim(50,110) +
...
#For family level
p.f_age <- ggplot(bracken.ctds.family, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
  ylab("normalized alpha diversity") +
    ylim(-1,0) + 
...
#For order level
p.o_age <- ggplot(bracken.ctds.order, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() +
  geom_smooth(method="lm") +
    ylim(-1,0) + 
  xlim(50,110) +
...
#For class level
p.c_age <- ggplot(bracken.ctds.class, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() + 
  geom_smooth(method="lm") +
    ylim(-1,0) + 
  xlim(50,110) +
...
#For phylum level
p.p_age <- ggplot(bracken.ctds.phylum, aes(x=Age_at_FecalSample, y=statistic)) + 
    geom_point() +
  geom_smooth(method="lm") +
  ylab("normalized alpha diversity") +
    ylim(-1,0) + 
...
png(paste0(outpath,"Figures/incommon_taxa/entropy_bracken_incommon.taxa_ilo_alllevels.png"), units="in", width=30, height=20, res=300)
ggpubr::ggarrange(p.p_age, p.c_age, p.o_age, p.f_age, p.g_age, p.s_age,
           ncol = 3, nrow = 2)
 dev.off()
 
 png(paste0(outpath,"Figures/incommon_taxa/entropy_bracken_incommon.taxa_ilo_main.png"), units="in", width=30, height=10, res=300)
ggpubr::ggarrange(p.p_age, p.g_age, p.s_age,
           ncol = 3, nrow = 1)
 dev.off()
```
...
#save figures
png(paste0(outpath,"Figures/incommon_taxa/entropy_methodcomp_incommon.taxa_ilo_alllevels.png"), units="in", width=30, height=20, res=300)
ggpubr::ggarrange(p.p_comp, p.c_comp, p.o_comp, p.f_comp, p.g_comp, p.s_comp,
           ncol = 3, nrow = 2)
dev.off()
 
png(paste0(outpath,"Figures/incommon_taxa/entropy_methodcomp_incommon.taxa_ilo_main.png"), units="in", width=30, height=10, res=300)
